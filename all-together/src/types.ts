...
export type Person = { id: string; name: string; ageGroup?: 'adult' | 'child'; accountUserId?: string };
...
export type Expense = { id: string; title: string; payer: string; amount: number; splitMode: 'equal' | 'selected' | 'custom'; participants: string[]; settled: boolean };
...
export type Photo = { id: string; title: string; uploader: string; placeholder: string };
export type Event = {
  id: string;
  title: string;
  type: string;
